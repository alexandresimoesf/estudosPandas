...
# 203	"melissa Spagnol Abraham"
# 210	"melissa Sales"
# 212	"melissa Acioli de Queiroz "
...
agenda_do_medico = read_csv('MELISSA.csv', sep=';', encoding='latin-1', low_memory=False)
...
agenda_do_medico = agenda_do_medico.groupby(['fk_paciente_id', 'data_agendada']).first().reset_index()
...
agenda_do_medico.to_csv('csv/melissa_agenda_dermacapelli.csv', index=False)
...
prontuario_csv.to_csv('csv/melissa_prontuario_dermacapelli.csv', encoding='UTF8', index=False)
...
prontuarioPermissao_csv.to_csv('csv/melissa_prontuarioPermissao_dermacapelli.csv', encoding='UTF8', index=False)